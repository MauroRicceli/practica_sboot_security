...
import com.example.prac_sboot_security.sbootsecurity.services.JwtService;
...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    private final RepositoryTokens tokenRepository;
    ...
    protected void doFilterInternal(
        ...
        //EL TOKEN EN EL HEADER EMPIEZA LUEGO DE 'BEARER '
        final String jwtToken = authHeader.substring(7);

        final String userEmail = jwtService.extractUsername(jwtToken);
        //VERIFICO SI EXISTE EN MI DB
        if(userEmail == null || SecurityContextHolder.getContext().getAuthentication() != null){
            return;
        }

        final TokenEntity token = tokenRepository.findByToken(jwtToken).orElse(null);
        //VERIFICO SI EL TOKEN EXISTE EN MI DB, Y SI SIGUE SIENDO VALIDO.
        ...
        final Optional<UserEntity> user = repositoryUsers.findByEmail(userDetails.getUsername());
